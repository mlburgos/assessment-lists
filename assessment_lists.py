...
# is the following still considered "readible"?

    return sorted(list(set(foods1) & set(foods2)))

# would this be a better solution wrt to clarity?

    # common_foods_set = set(foods1) & set(foods2)

    # return sorted(list(common_foods_set))
...
# I sort the list in reverse order, slice the desired number of integers,
# and then resort them in ascending order before returning

    return sorted(sorted(items, key=None, reverse=True)[:n])

# Is it more readible to split it into two steps like this?

    # items_in_descending_order = sorted(items, key=None, reverse=True)

    # largest_n = items_in_descending_order[:n]
